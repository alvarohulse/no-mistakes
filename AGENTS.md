...
- The daemon runs `commands.*` from `.no-mistakes.yaml` verbatim via `sh -c`, `agent` selects which process launches (incl. `acp:` targets) with the maintainer's credentials, and `prompts` steers that launched agent. To prevent supply-chain RCE, the **code-executing and agent-steering fields (`commands.{test,lint,format}`, `agent`, and `prompts`)** are loaded from the trusted default branch, never from the pushed SHA. See `internal/daemon/manager.go` `startRun` + `loadTrustedRepoConfig`, and `config.EffectiveRepoConfig`.
- `startRun` fetches the default branch, resolves it to an exact commit SHA (`git.ResolveRef`), and `loadTrustedRepoConfig` reads `.no-mistakes.yaml` at that **pinned SHA** (not the `origin/<defaultBranch>` ref name). On fetch failure (or if the ref does not resolve) the trusted SHA is empty → `loadTrustedRepoConfig` returns nil → `EffectiveRepoConfig` forces empty `commands`/`agent`/`prompts`. This fails closed: a stale `origin/<default>` ref left in the shared bare repo by a previous run cannot serve a value the live default branch removed. Regression tests: `TestLoadTrustedRepoConfig_FailClosedOnFetchFailure`, `TestLoadTrustedRepoConfig_PinnedSHAReadsFreshDefaultBranch`.
- Non-steering fields (`ignore_patterns`, `auto_fix`, `intent`, `test`) are still read from the pushed branch.
- `allow_repo_commands` is **per-repo, read from the trusted default-branch copy of `.no-mistakes.yaml`** (declared on `RepoConfig`), never the global config and never the pushed SHA. It defaults `false`; when `true` the maintainer has opted in to honoring the pushed branch's `commands`, `agent`, and `prompts` wholesale. A contributor cannot self-enable it from a pushed branch. When changing this logic, keep `commands`/`agent`/`prompts` locked to the default branch and update the e2e test `TestRepoConfigCommandsFromDefaultBranch` (incl. the `pushed_branch_cannot_self_enable` subtest and its hostile-`prompts` marker assertions).
...
**Prompt Additions (`prompts` config)**

- `prompts` (`config.PromptConfig`, global + repo) is **append-only** steering for the pipeline's model prompts: built-in prompts stay authoritative for structure, output schemas, safety rules, and worktree boundaries. `PromptConfig.SectionForStep` renders the configured text as a trailing "Additional prompt config" section (empty config renders nothing); never let configured text replace or precede a built-in prompt.
- Append order is fixed: global `shared`, repo `shared`, global `<step>`, repo `<step>` (`mergePromptConfigs` in `Merge` + `PromptConfig.ForStep`). Step keys: `intent`, `rebase`, `review`, `test`, `document`, `lint`, `pr`, `ci`. Pipeline steps consume it via `configuredPromptSection` (`internal/pipeline/steps/prompt_config.go`); the intent step passes the rendered section into `intent.NewAgentSummarizer`/`NewAgentDisambiguator`.
- The intent summary cache key includes the rendered prompt section (`intent.cacheKeyFor`), so a prompt-config change invalidates cached summaries instead of serving text produced under the old prompt. Keep `ExtractParams.PromptSection` identical to the section the summarizer/disambiguator were built with.
- The setup wizard's branch-name/commit-subject suggestion prompts (`internal/agent/suggest.go`) are not pipeline prompts and intentionally do not receive `prompts` additions.
