...
- The daemon runs `commands.*` (including `commands.build`) and `hooks.{post_worktree,pr_body}` from `.no-mistakes.yaml` verbatim via `sh -c`, while the run-wide `agent`, every `<step>.agent`/`<step>.model` route, and the Review adversary route (`review.adversary_agent`/`review.adversary_model`) select which processes and models launch with the maintainer's credentials. `prompts.*` (append-only per-step prompt additions) executes nothing but steers those launched agents, so it sits behind the same boundary and the same `allow_repo_commands` opt-in. These fields are loaded from the trusted default branch at a **pinned SHA** resolved by a fresh fetch, never from the pushed SHA. The run aborts when the trusted commit or its present config cannot be read and parsed; a readable tree with no config is valid. See `internal/daemon/manager.go` `startRun`, `loadTrustedRepoConfig`, and `assertGateTrustedConfigReadable`.
...
- `codex exec resume` has a narrower flag surface than `codex exec`, so an unsupported override fails the resume and falls back; both forms use `-` to read prompts from stdin, and the e2e fakeagent must keep parsing both argv shapes (`extractCodexPrompt`).
