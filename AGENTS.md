...
- On Windows, call `shellenv.HideWindow(cmd)` on any direct `os/exec` spawn that bypasses `ConfigureShellCommand` (git one-shots, pipeline `stepCmd`, managed agent servers, internal `taskkill` cleanups). Without it, each console subprocess pops its own window. `ConfigureShellCommand`, `git.Run`, and `stepCmd` already apply it. No-op off Windows. Regression: `TestHideWindow_*` (windows build tag).
