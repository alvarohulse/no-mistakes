...
C1 deliberately refuses every model route whose resolved agent is an ACP target or alias, including `cursor`. C2 will add the missing ACP spawn plumbing; until then, failing before work begins prevents a configured model from silently normalizing or disappearing.
