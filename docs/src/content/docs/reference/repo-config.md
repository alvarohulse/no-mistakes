...
When empty, the agent selects one exact focused test command in a read-only planning pass; the pipeline executes and records it, then an evidence agent gathers evidence and artifacts, running further focused checks itself when the planned command alone does not demonstrate the intent. When user intent is available, the evidence agent may also run after a configured baseline succeeds, still under the same targeted-validation contract. See the [Test step reference](/no-mistakes/reference/pipeline-steps/#test) for repair behavior.
