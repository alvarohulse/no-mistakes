...
Setting the variable is the opt-in. When it is unset, config loading and recovery retain their existing behavior. launchd, systemd, and Windows Task Scheduler definitions forward the current value, and a later daemon start or restart removes a stale entry after the variable is unset. The Windows task action sets only `NM_REPO_CONFIG`; it does not persist proxy variables or credentials.
