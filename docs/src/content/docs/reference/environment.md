...
Setting the variable is the opt-in. When it is unset, config loading and recovery retain their existing behavior. launchd, systemd, and Windows Task Scheduler definitions forward the current value, and a later daemon start or restart removes a stale entry after the variable is unset. The Windows task action points to a generated launcher under `NM_HOME` that sets only `NM_REPO_CONFIG`; it does not persist proxy variables or credentials. The launcher is replaced atomically and stale launchers are removed only after the task replacement succeeds.
...
When the daemon runs through a managed service (launchd, systemd user service, Task Scheduler), the macOS and Linux service definitions include a default `PATH` with common user and system binary directories. All three platforms forward the current `NM_REPO_CONFIG` value. macOS and Linux also bake in any proxy variables (`HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY`, `ALL_PROXY`) set when the service is installed or refreshed; the Windows task deliberately does not persist proxy variables or credentials. Proxy values are preserved across later macOS/Linux service refreshes and restarts when absent from the current shell; `NM_REPO_CONFIG` is not preserved on any platform, because removing the variable disables that explicit opt-in. Both upper- and lower-case proxy spellings are forwarded exactly as set. Because a proxy URL can embed credentials, generated macOS/Linux service files are restricted to owner-only `0600` permissions whenever proxy values are present; a machine-config path alone keeps the conventional `0644` mode. At daemon startup, the daemon resolves environment from the login shell on macOS and Linux, preserves its `PATH` order, and appends missing well-known directories such as `~/.local/bin`, `~/go/bin`, `~/.cargo/bin`, `~/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin`, and `/bin`. If login-shell resolution fails or returns no entries, the daemon logs a warning and uses an augmented process-environment fallback that may omit version-manager directories such as nvm, fnm, or volta. On Windows it reuses the current process environment in addition to the task action's explicit machine-config setting.
