...
Inspect the evidence with `no-mistakes stats --agents` (per-purpose aggregates, including a `METRICS` coverage count so a real zero is distinguishable from missing instrumentation) or `no-mistakes stats --run <id>` (one run's steps, content-free command/runner and repair-progress receipts, skip and Review-route receipts, per-step invocations, invocation modes and nested identities, the per-round-vs-cumulative token split, and parked time). Add `--format json` for the normative local machine contract, or use the direct repository/time/step/agent/model/purpose/status filters with text, JSON, or long-form CSV projections.
