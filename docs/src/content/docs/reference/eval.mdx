...
The report is deliberately cautious. It never treats an unadjudicated candidate finding as a false positive, excludes candidates with failed replays from the frontier, and distinguishes missing token or cost instrumentation from a real zero. Captured source invocation records also preserve the adapter's explicit `complete` or `unknown` usage coverage; capture does not infer coverage from token presence. Stored historical evaluations remain immutable inputs: reporting never replays, reprices, or recomputes them.
