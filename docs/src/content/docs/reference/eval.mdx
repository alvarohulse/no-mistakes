...
Capture writes gold from recorded gate evidence: human Fix and add-finding
decisions, plus the merge-derived auto-fix and shipped-unfixed rules below. It
reads the normalized round graph; the legacy field names below are compatibility
projections of those records. A merged PR is not a case-level pass or fail:
...
- On an unmerged PR, an approved findings gate's explicit-empty `user_declined`
  waiver, a skip or abort inferred from the step outcome, and a round with no
  recorded decision stay **unlabeled / pending** until stronger evidence exists.
