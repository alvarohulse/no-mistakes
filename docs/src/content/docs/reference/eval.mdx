...
Repeating a replay creates a new measurement session, but the same case IDs and repeat count produce the same cohort. Replay never mutates the frozen case manifest, labels, or replay configuration. Set inspection, diversified refresh, and report reads likewise stabilize when repeated.
