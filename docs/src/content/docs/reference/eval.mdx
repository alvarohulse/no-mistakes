...
When `eval sets`, `eval report`, or `eval run` resolves repository fingerprints into display names, it consults `state.sqlite` only when that pipeline database already exists and opens it read-only. Display lookup never creates or migrates pipeline state; without a readable database, dashboards fall back to short fingerprints.
