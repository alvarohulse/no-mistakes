...
This snapshot is descriptive, not a benchmark. It contains 35 cases across the full local corpus index. The default `eval.diversified_size` cap is 32 and applies only to the gold-only diversified holdout, so it is intentionally separate from the corpus total shown here.
