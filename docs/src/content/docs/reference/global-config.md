...
- Older unpinned rich rows are replaced atomically with content-free metric receipts, then their evidence, run launch artifacts, and run logs are removed. Each receipt keeps cleanup pending until deletion succeeds, so a crash, filesystem error, or invalid stored cleanup target remains retryable without blocking cleanup of other archived runs on the next retention pass.
