...
- Older unpinned rich rows are replaced atomically with content-free metric receipts, then their evidence, command-output artifacts, run launch artifacts, and run logs are removed. Each receipt keeps cleanup pending until deletion succeeds, so a crash, filesystem error, or invalid stored cleanup target remains retryable without blocking cleanup of other archived runs on the next retention pass.

Reaping runs after each finished run and again at daemon startup. Use `no-mistakes runs pin <run-id>` and `unpin` to change explicit retention. Before rich rows cascade away their steps, rounds, command definitions, command attempts, and invocations, no-mistakes atomically stores an immutable, no-foreign-key metric receipt so historical stats survive. The [local/remote data boundary](/no-mistakes/reference/environment/#what-stays-local-and-what-leaves-the-machine) owns that receipt's content-free fields and privacy exclusions. An upgraded daemon also drains the pre-relocation directory in the system temp directory under the same rules; nothing is migrated, because absolute paths recorded in older pull request bodies name the old location.
