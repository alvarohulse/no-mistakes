...
- Older unpinned rich rows are replaced atomically with content-free metric receipts, then their evidence, run launch artifacts, and run logs are removed. The receipt keeps cleanup pending until deletion succeeds, so a crash or filesystem error is retried on the next retention pass.
