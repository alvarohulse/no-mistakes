...
Only a controller- or provider-observed Build, Test, or Lint attempt that completes with exit zero and an unchanged declared tested commit becomes testing proof. Failed, interrupted, stale, or agent-reported attempts remain visible in pipeline history but never certify a commit.
