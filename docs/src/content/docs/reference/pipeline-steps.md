...
- If the agent creates a test file or modifies an existing test file, the step records a warning with `action: ask-user` and requires approval even when tests pass. Detection covers untracked, staged, and unstaged changes in the agent's working tree. This finding requires an explicit response: unlike an ordinary `ask-user` finding, TUI yolo mode and AXI `--yes` do not auto-resolve it, so the run stays parked until a human or driving agent responds to it directly.
