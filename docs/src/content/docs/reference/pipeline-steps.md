...
Configured [`prompts`](/no-mistakes/reference/repo-config/#prompts) guidance, shared or per step, is appended to each model-invoking step's prompt as extra steering; built-in prompts stay authoritative.
