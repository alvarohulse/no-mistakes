...
- PR body includes a `## Intent` section when user intent is available, an operator-supplied `## Notes` section when the run was started with `axi run --pr-note` or `--pr-note-file` (placed after Intent and before What Changed, reproduced verbatim after surrounding whitespace is trimmed, and fed to the PR summary as trusted author guidance), an agent-authored `## What Changed`, and regenerated `## Risk Assessment`, `## Testing`, and `## Pipeline` sections from recorded step results and rounds; a note that already starts with `## Notes` is not given a duplicate heading; auto-fix results in `## Pipeline` render as an issue -> fix -> verification narrative using captured fix summaries, re-check success text, and any still-open findings
...
- Intent, author `## Notes`, `## What Changed`, risk, and testing sections are kept ahead of pipeline history; if those sections or the newest pipeline update are still too large, the PR step truncates at line or section boundaries and adds an explicit marker. The author `## Notes` section is a normal section placed after `## Intent` with no note-specific truncation handling: the Pipeline section is clamped first, but if the body still exceeds the limit the note is truncated along with the rest of the body rather than being byte-protected.
...
- For Azure DevOps, the PR description is capped at 4000 characters (UTF-16 code units, matching .NET's measurement): the agent is told about the cap and asked to keep the `## What Changed` section compact. An oversized body drops `## Testing` first, preserving the Intent, (author `## Notes`,) What Changed, Risk Assessment, and Pipeline narrative before the final connector-level clamp keeps the body within the cap. Because the connector-level clamp keeps content from the top of the body, the author note (placed right after Intent) is preserved in practice unless Intent plus the note alone exceed the cap.
