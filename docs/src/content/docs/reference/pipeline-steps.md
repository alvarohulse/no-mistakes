...
Each configured or planned command that reaches terminal controller observation is atomically linked to exactly one complete immutable output artifact, including a real zero-byte file when the command emits no output. [The Gate Model](/no-mistakes/concepts/gate-model/#database) owns its storage and integrity contract.
...
Only the controller can accept a Build, Test, or Lint attempt as validation proof. It accepts only a complete controller- or provider-observed pass with exit zero, no signal, and an immutable output artifact, whose declared tested SHA exactly matches the clean, unchanged commit observed before and after execution. Proof is grouped by that exact SHA; it never implies ancestry or coverage of the current head. Failed, errored, cancelled, timed-out, waived, stale, superseded, incomplete, or agent-reported attempts remain pipeline history but never certify a commit. An approved Test step whose commands all fail retains its attempt and output history but has an empty proof set.
...
- Records one indexed Refresh operation receipt for each target decision, including the source and destination refs, authoritative base ref and (after successful fetch and resolution) SHA, starting and resulting heads, strategy, decision, conflict state, repair state, and elapsed time
- Links each controller-run primary reset, rebase, or merge attempt to its immutable command-output artifact; failed primary operations remain linked when an agent repairs the conflict
- Refusals and controller errors carry a bounded redacted diagnostic artifact when no primary command output is available
...
- When a [Review candidate pool](/no-mistakes/reference/repo-config/#per-step-agent-model-and-effort-routes) is configured, selects one usable harness/model/effort route uniformly for every full review, records the final pool and selection, and applies the installed `/review-changes` contract; the ordinary Review route remains the fixer
...
- If `commands.test` is empty, first asks the routed Test agent for one exact targeted command in a read-only planning pass, then executes and records it. After it passes, an evidence agent gathers evidence and artifacts for the intent; it may run further focused checks or write a focused test itself, but never the complete repository suite. A failure normally reruns the same planned command after repair. Only when the repair agent explicitly identifies that persisted plan as invalid may it supply a corrected replacement; the controller persists, executes, and records that replacement through the normal validation path.
