...
An explicit list replaces any machine-owned configured skip list for that repository. Use `--skip=none` to clear configured skips for one run.
Valid step names are `intent`, `refresh`, `review`, `build`, `test`, `document`, `lint`, `push`, `pr`, and `ci`.
...
| `-y`, `--yes` | `bool`   | `false` | Auto-resolve every eligible gate until a decision point or outcome             |
...
Use `--agents` for the per-purpose aggregate tables followed by the detailed text projection of the same report. The tables summarize session modes, duration, nullable per-round token totals, subprocess time, model round-trips, and tool activity; JSON and CSV carry each nullable aggregate meter's selected-invocation coverage. The detail lists individual top-level invocation facts, including duration, session mode, explicit `complete` or `unknown` usage coverage, nullable per-round and raw token meters, and workload.
Use `--run <id>` to inspect one run's binary/build identity, ordered steps and rounds, content-free command/runner and repair-progress receipts, configured skip receipts, policy and config digests, Review candidate pool and selected route, and individual top-level agent invocations. The text view's `rounds=` count includes completed rounds only; JSON and CSV include each round's status. Invocation detail includes the step, purpose, provider/model, configured reasoning effort, explicit usage coverage, per-round token deltas next to raw counters, CLI-reported cost, tool-category breakdown, workload size, finding count, and fallback reason. It also shows the nullable total time parked at approval gates and implies `--agents`.
Dashboard and aggregate finding counts use completed-round findings; when a step has no completed rounds, they use its last recorded step findings.
...
`--format` accepts `text` (the default), canonical `json`, or long-form `csv`; every format is derived from the same `Report` envelope. JSON is the normative, versioned machine contract. CSV emits exactly one row for every JSON leaf, including nulls and empty collections, and its `json_path` column identifies that leaf; record, section, entity, coverage, basis, and reason columns keep common queries direct. Aggregate per-round token and activity meters include coverage and remain `null` when incomplete; run-level aggregate delta-token facts also remain null when an independent database aggregate does not match the emitted invocation rows. `reported_cost_usd` is only an amount reported by the agent CLI: missing cost stays `null` in JSON, `—` in detailed text, and `not_reported` in CSV metadata. Stats never retain or derive list-price, harness-adjusted, or other estimated cost classes.
...
```sh
no-mistakes pr-body [--sample [--sample-version <2|3|5>] | --run <id> | --contract-file <path>] [--print-contract] [--hook <command>]
```

| Flag               | Type     | Default    | Description                                                     |
| ------------------ | -------- | ---------- | --------------------------------------------------------------- |
| `--sample`         | `bool`   | `false`    | Use the built-in contract that exercises every section           |
| `--sample-version` | `int`    | `5`        | Contract version `--sample` emits (`2`, `3`, or `5`)               |
...
Without a source flag it uses the latest run for the current repository. `--run` reuses the exact stored title, Summary, and What Changed for runs that recorded a PR narrative; older runs without one retain empty narrative fields. A run id belonging to another repository is rejected rather than mixed with this directory's `repo` block. `--contract-file` accepts any contract version this build still supports (`5`, `3`, or `2`). Version 5 carries raw invocation meters and optional CLI-reported cost without derived cost classes; the formatter owns pricing data and estimation. Version 4 is rejected because its producer-calculated cost receipts are no longer part of the contract.
