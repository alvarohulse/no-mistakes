...
`--format` accepts `text` (the default), canonical `json`, or long-form `csv`; every format is derived from the same `Report` envelope. JSON is the normative, versioned machine contract. CSV emits exactly one row for every JSON leaf, including nulls and empty collections, and its `json_path` column identifies that leaf; record, section, entity, coverage, basis, and reason columns keep common queries direct. Aggregate per-round token and activity meters include coverage and remain `null` when incomplete; run-level aggregate delta-token facts also remain null when an independent database aggregate does not match the emitted invocation rows. New rows retain raw usage and CLI-reported cost only. Historical three-class cost receipts remain available, with their API-list and harness-adjusted fields explicitly labeled as historical derived estimates; absent estimates remain unknown and are never recalculated or fabricated as `$0`.
...
Without a source flag it uses the latest run for the current repository. `--run` reuses the exact stored title, Summary, and What Changed for runs that recorded a PR narrative; older runs without one retain empty narrative fields. A run id belonging to another repository is rejected rather than mixed with this directory's `repo` block. `--contract-file` accepts any contract version this build still supports (`5`, `4`, `3`, or `2`). Version 5 carries raw invocation meters and optional CLI-reported cost without derived cost classes; the formatter owns pricing data and estimation. Version 4 remains readable for historical producer-calculated receipts.
