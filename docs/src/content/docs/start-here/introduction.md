...
flowchart LR
  ...
  worktree --> pipeline["intent -> rebase -> build -> review -> test -> document -> lint -> push -> pr -> ci"]
...
- A fixed, opinionated pipeline: `intent → rebase → build → review → test → document → lint → push → pr → ci`. Order is not configurable; what each step runs is.
- Choice of agent: `claude`, `codex`, `grok`, `rovodev`, `opencode`, `pi`, `copilot`, or `cursor` / `acp:<target>` via `acpx`, with per-repo override and ordered fallbacks; every gate requires a runnable configured pipeline agent.
