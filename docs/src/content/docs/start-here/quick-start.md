...
- `git`
- One supported agent runner (`claude`, `codex`, `cursor-agent`, `acli` for Rovo Dev, `opencode`, `pi`, or `copilot`), or an explicit ACP target; see [Global Config](/no-mistakes/reference/global-config/) for ACP requirements
...
2. **Refresh** - rebase (default) or merge onto the latest upstream and pushed-branch target
3. **Review** - AI code review of your diff
4. **Build** - compile the changed production code
5. **Test** - baseline tests plus evidence checks when intent is known
6. **Document** - updates docs and reports unresolved gaps
7. **Lint** - your linters (configured command, or one the agent plans when none is configured)
