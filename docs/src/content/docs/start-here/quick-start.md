...
For `agent: acp:<target>`, verify `acpx` or `acpx_path` separately because `no-mistakes doctor` only validates ACP aliases such as `cursor`, not arbitrary ACP targets.
