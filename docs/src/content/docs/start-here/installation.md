...
- **git** - required
- **One supported agent binary** - `claude`, `codex`, `acli` (Rovo Dev), `opencode`, `pi`, or `copilot`; `cursor-agent` with `acpx` for `agent: cursor`; or a separately installed `acpx` binary for `agent: acp:<target>`
