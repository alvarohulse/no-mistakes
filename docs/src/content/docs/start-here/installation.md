...
- **git** - required
- **One supported agent runner** - `claude`, `codex`, `cursor-agent`, `acli` (Rovo Dev), `opencode`, `pi`, or `copilot`, or an explicit ACP target; see [Global Config](/no-mistakes/reference/global-config/) for ACP requirements
...
Run `no-mistakes doctor` to check native agents, the registered `acp:cursor` fallback, provider tools, and whether the configured global runner can start a validation gate.
