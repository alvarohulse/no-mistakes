...
- A **round** is one persisted execution pass of a step. It starts `active`
  and ends `completed` or `failed`; it links the related agent invocations,
  command attempts, and owner-local artifacts. Structured rounds (every step
  except Refresh and Push) also retain the subject's starting, resulting,
  evaluated, and trusted-config commit identities when available.
- An **evaluation** is the stable, explicitly typed result of a structured
  round. Its kind is `initial_review`, `rereview`, `validation`,
  `revalidation`, or `documentation`.
- A **finding** belongs to exactly one evaluation. Findings retain stable,
  run-unique record identities, an agent-facing ID unique within their
  evaluation, and their original order. User-added findings become records in
  the same evaluation rather than a copied fix payload.
- A **decision** records a selection against an evaluation's finding
  identities: selected findings keep selection order, while every other
  finding remains deliberately unselected. It also retains per-finding user
  edits and the decision source. A **waiver** is an explicit empty decision
  that resolves a findings gate; a missing decision leaves it unresolved.
  Skip and abort retain their terminal decision provenance but end the step or
  run instead of resolving it.
- A **repair** is the receipt for repair progress on its owning round. It
  retains the fix summary, a content-free failure fingerprint, result
  (`attempted`, `resolved`, `stopped_no_progress`,
  `stopped_repeated_failure`, or `stopped_attempt_limit`), and resulting head
  when available.
- An **initial round** is a step's first pass. A **fix round** follows an
  ...
  explicit legacy provenance. Migrations add records only and never infer
  missing history.
