...
Each automatic or user-selected repair is a later fix round. The
[pipeline execution vocabulary](/no-mistakes/concepts/pipeline/#execution-vocabulary)
owns the round and fix-round terms; the [Gate Model](/no-mistakes/concepts/gate-model/#database)
owns their persisted records.
