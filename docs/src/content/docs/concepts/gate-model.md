...
Step rounds begin as `active` before their work starts and finish as `completed` or `failed`. They record each execution pass (initial, auto-fix) with its findings and duration, plus selected finding IDs, whether the selection came from the user or auto-fix filtering, the merged finding payload actually sent to the fix agent for that round, and the one-line fix summary for fix rounds. Only completed rounds contribute findings, repair receipts, eval capture, and sanitized completed-round history.
