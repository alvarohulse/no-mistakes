...
Command definitions deduplicate a run's resolved script, platform, and canonical runner executable and arguments; resolved executable paths and runner versions do not change that portable identity. Each command attempt records one controller-observed execution with its step, round, sequence, purpose, trigger, subject, input/result state, timing, outcome (`pass`, `fail`, `process_error`, `cancelled`, or `timeout`), exit code or signal, and any retry link. A terminal attempt is atomically linked to exactly one immutable output artifact under `runs/<runID>/command-output/<attemptID>.log`; its registry entry carries a normalized root-relative path, source-byte count, and SHA-256 digest, all of which are rechecked before the file is read. The attempt starts before launch and completes when the controller observes process termination; a missing outcome means completion was not durably observed, such as after a daemon crash. This does not observe work that outlives the managed process boundary.
