...
6. The pipeline runs in order: `intent -> refresh -> review -> build -> test -> document -> lint -> push -> pr -> ci`. The refresh step displays as Rebase or Merge for its persisted strategy.
...
8. After local checks pass, the push step forwards the branch to the configured push target only after verifying that the update will not discard unincorporated commits already on that target, and the PR step creates the pull request or updates only verified no-mistakes-owned sections in the branch's existing one while preserving every unowned byte.
...
- **Fixed pipeline** - the step order is opinionated and not configurable: `intent → refresh → review → build → test → document → lint → push → pr → ci`. What you _can_ configure is the commands each step runs, how many auto-fix attempts are allowed, the refresh strategy, and whether transcript-based intent extraction is used when intent is not supplied directly.
...
The worktree is where `no-mistakes` can safely refresh by rebase or merge, run commands, let the
...
The executor runs each step sequentially and manages the approval/fix loop. It can also end early after `refresh` if the branch has no diff against its selected base branch, marking the remaining steps as skipped.
...
Command definitions deduplicate a run's resolved script, platform, and canonical runner executable and arguments; resolved executable paths and runner versions do not change that portable identity. Each command attempt records its observer and its step, round, sequence, purpose, trigger, subject, input/result state, timing, outcome (`pass`, `fail`, `process_error`, `cancelled`, or `timeout`), exit code or signal, and any retry link. A terminal attempt is atomically linked to exactly one immutable output artifact under `runs/<runID>/command-output/<attemptID>.log`; its registry entry carries a normalized root-relative path, source-byte count, and SHA-256 digest, all of which are rechecked before the file is read. Only controller completion can set the paired `accepted_as_proof` and `proof_reason` fields; [Pipeline Steps](/no-mistakes/reference/pipeline-steps/) owns the validation criteria and exact-SHA scope. A controller-executed attempt starts before launch and completes when the controller observes process termination; a missing outcome means completion was not durably observed, such as after a daemon crash. This does not observe work that outlives the managed process boundary.
Refresh target decisions and controller-owned Push decisions, including CI repairs, have separate indexed operation receipts constrained to the same run, owning step, and round; linked primary command attempts and diagnostics must share that scope. A Push receipt starts before controller inspection or transport, preserving its target, decision, and observed evidence through later failures. Receipts preserve primary command-attempt order and can own a bounded immutable redacted diagnostic artifact. A completed Refresh receipt is atomically inserted with a new diagnostic-artifact registration, or atomically claims a compatible pre-registered diagnostic; a Push receipt is instead terminalized from its preallocated row. Operation receipts and their artifacts cascade with their owning run, step, and round. [Daemon & Worktrees](/no-mistakes/concepts/daemon/#crash-recovery) owns how incomplete Push receipts are terminalized after a crash.
Persistence failures while creating a round, saving a command definition or attempt, or completing an attempt fail the step closed rather than continue with incomplete provenance.
[Pipeline](/no-mistakes/concepts/pipeline/#execution-vocabulary) owns the durable round, evaluation, finding, decision, repair, and legacy-compatibility vocabulary.
...
Each top-level agent invocation records local-only performance evidence. The [environment reference](/no-mistakes/reference/environment/#what-stays-local-and-what-leaves-the-machine) owns its fields, coverage semantics, and privacy boundary. Full Review rows retain the content-free candidate pool used for that selection; prompts, outputs, diffs, paths, and credentials are never stored there.
Use `no-mistakes stats` for the bounded dashboard, `no-mistakes stats --agents` for per-purpose aggregates plus detailed invocation facts, or `no-mistakes stats --run <id>` for one run's report and parked time. These reports project content-free command receipts from step evidence; they do not expose the rich command-definition or command-attempt graph. Direct selectors scope dashboard and aggregate facts together. Rich rows and artifacts follow the [`test.evidence` retention policy](/no-mistakes/reference/global-config/#local-storage-and-cleanup); older runs keep an immutable content-free metric receipt outside the run/repository cascade so privacy-safe stats history survives.
...
| `runs/<runID>/`                  | Owner-only immutable launch configuration, integrity metadata, and command-attempt output artifacts                    |
