...
5. The pipeline runs in order: `intent -> rebase -> review -> test -> document -> retrospect -> lint -> push -> pr -> ci`.
...
- **Fixed pipeline** - the step order is opinionated and not configurable: `intent → rebase → review → test → document → retrospect → lint → push → pr → ci`. What you _can_ configure is the commands each step runs, how many auto-fix attempts are allowed, whether the optional retrospective step runs, and whether transcript-based intent extraction is used when intent is not supplied directly.
