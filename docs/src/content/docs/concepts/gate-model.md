...
Each agent invocation records local-only purpose, provider/model metadata, session mode and a truncated session-identity hash, top-level harness invocation mode, wire-observed nested agent identities, timing, failure category, raw token usage, and optional CLI-reported cost. Rows written by older versions may also retain an immutable historical pricing receipt, which remains readable but is never recalculated. Full Review rows retain the content-free candidate pool used for that selection; prompts, outputs, diffs, paths, and credentials are never stored there.
