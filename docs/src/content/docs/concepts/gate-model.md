...
Refresh target decisions have a separate indexed operation receipt that is constrained to the same run, Refresh step, and round. It preserves primary command-attempt order and can own a bounded redacted diagnostic artifact. A receipt and a newly created diagnostic-artifact registration are inserted atomically; operation receipts and their artifacts cascade with their owning run, step, and round.
