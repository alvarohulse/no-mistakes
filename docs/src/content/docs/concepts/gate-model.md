...
Newly completed rounds store a structured graph: one evaluation with its
ordered findings and result artifacts, a decision that references selected and
unselected findings, and a repair receipt. User-added findings and per-finding
edits remain associated with the evaluation and its decision references.
The round retains its starting, resulting, evaluated, and trusted-config
commits when available, plus links to the associated agent invocations,
command attempts, and registered artifacts.
Structured rows leave the former embedded JSON round fields empty; readers
produce compatibility projections only for older consumers. The additive
migration leaves existing rows in their original form without inventing
evaluations, findings, decisions, repairs, or provenance.
[Pipeline execution vocabulary](/no-mistakes/concepts/pipeline/#execution-vocabulary)
owns the round kinds, trigger terms, legacy `user_fix` term, and waiver
semantics.
