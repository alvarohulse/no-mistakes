...
Each agent invocation records local-only purpose, provider/model metadata, session mode and a truncated session-identity hash, top-level harness invocation mode, wire-observed nested agent identities, timing, failure category, token usage, and a write-once content-free pricing receipt. Full Review rows also retain the content-free candidate pool used for that selection; prompts, outputs, diffs, paths, and credentials are never stored there.
Use `no-mistakes stats` for the bounded dashboard, `no-mistakes stats --agents` for per-purpose aggregates plus detailed invocation facts, or `no-mistakes stats --run <id>` for one run's report and parked time. Direct selectors scope dashboard and aggregate facts together. Active and pinned runs, every run newer than 14 days, and the newest 50 terminal unpinned runs retain rich rows and artifacts. Older runs keep an immutable content-free metric receipt outside the run/repository cascade, so stats history survives without retaining prompts, outputs, findings prose, paths, diffs, command text, resolved executable argv, or session identities. The receipt does retain categorical command outcomes and validated bare-shell runner provenance plus repair progress facts; `no-mistakes runs pin <run-id>` changes explicit retention.
