...
Refresh target decisions have a separate indexed operation receipt constrained to the same run, Refresh step, and round; linked primary command attempts and diagnostics must share that scope. It preserves primary command-attempt order and can own a bounded immutable redacted diagnostic artifact. A receipt is atomically inserted with a new diagnostic-artifact registration, or atomically claims a compatible pre-registered diagnostic; operation receipts and their artifacts cascade with their owning run, step, and round.
