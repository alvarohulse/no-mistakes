...
To inject your own content into the pull request the `pr` step opens, pass `--pr-note "<text>"` (or `--pr-note-file <path>` for longer content; the two are mutually exclusive and each input is limited to 47 KiB).
...
When the PR body must be truncated to fit a host's character limit, the generated and pipeline sections are clamped before the author note; the note itself is clamped only when it alone exceeds the host limit.
