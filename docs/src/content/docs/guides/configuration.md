...
Commands accept either a scalar or the shared structured runner form. Preflight consumes runner/platform overrides now; the existing Build/Test/Lint/Format call sites continue using the base `run` string and their legacy platform shell until the later pipeline migration.
