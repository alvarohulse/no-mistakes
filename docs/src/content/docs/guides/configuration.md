...
- `prompts` never overrides: global and repo guidance append in order - global `prompts.shared`, repo `prompts.shared`, global `prompts.<step>`, repo `prompts.<step>` - and built-in prompts stay authoritative.
