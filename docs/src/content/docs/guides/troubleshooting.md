...
Requested test evidence may still be written under the managed evidence directory (`<NM_HOME>/evidence/<run-id>` by default). It remains owner-local and is never published to a Git branch or uploaded as a pull-request attachment; the [Global Config Reference](/no-mistakes/reference/global-config/#testevidence) documents retention and migration from the removed publication keys.
