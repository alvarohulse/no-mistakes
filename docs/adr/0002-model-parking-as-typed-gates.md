...
An explicit trusted opt-in may add **Publish diagnostic PR** to durable Review, Build, Test, Lint, and final-verification gates. The action safely publishes the frozen committed head through the normal drafting and PR ownership path, requires a no-mistakes-owned Risk Assessment marked **Diagnostic — not certified** (distinct from the existing `Uncertified` review-provenance range), and leaves the original run parked. Diagnostic PRs default to Draft, but Draft is configurable and is not the source of certification truth.
...
- **Use Draft as the diagnostic signal:** rejected because Draft is provider-dependent and policy-overridable. Verified **Diagnostic — not certified** Risk Assessment content is the invariant.
