...
Optional `--pr-note` (or `--pr-note-file <path>` for longer content; mutually exclusive) injects your own text into the pull request the `pr` step opens or updates. Each raw input is limited to 16 KiB before trimming; the note, metadata, `--intent`, and any `--skip` options are bounded together so the git push-option command fits on every platform. After surrounding whitespace is trimmed, the note is reproduced verbatim in the PR body and is also fed to the PR summary prompt as trusted author guidance. In the built-in body it becomes the leading `## Notes` section before `## Summary`, and a note that already opens with a `## Notes` heading (case-insensitive) does not get a second one; when a `hooks.pr_body` formatter is configured, the note reaches it as trusted data and that formatter decides where the note goes. Existing pull requests are updated only through valid no-mistakes-owned sections; human and third-party bytes outside those markers remain unchanged. The locally operator-typed note is trusted, so it receives no adversarial stripping and reaches the PR summary prompt and any `hooks.pr_body` formatter unredacted; final hosted candidates reject possible secrets - do not include them. The flags apply only when starting a new run. The check keys off flag presence, including an empty value; `axi run` fails if it cannot verify active-run state and rejects reattachment before opening a note file. The note persists on the new run and is carried into `axi run`'s fallback rerun when the gate already has the same head, but `no-mistakes rerun` and the TUI rerun start a fresh run without it.

Optional `--metadata <string>` stores one opaque UTF-8 string up to 16 KiB. no-mistakes does not parse it as JSON, keys, or associations: pipeline subprocesses receive the exact value as `NM_METADATA`, agent prompts receive a sanitized copy marked as untrusted, and PR body contract v4 exposes the original string to formatters. Treat metadata as non-secret input. `no-mistakes rerun` inherits the previous value when the flag is absent; pass an explicit empty value to clear it.
