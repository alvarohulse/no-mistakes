...
- Do not hand-edit `CHANGELOG.md` or `.release-please-manifest.json`. During the provenance-guard bootstrap, the legacy pull-request check rejects any PR that touches either file unless its author is one of the two exempted release bots. The base-controlled check validates canonical upstream before any fast path, so fetch, ref, or ancestry failures fail closed even for source-only PRs. Its pinned historical exception trusts the earliest base first-parent commit that descends from the bootstrap only when that adoption anchor preserves the bootstrap entries exactly. Later base first-parent transitions and side histories are audited without fixed lifetime ceilings, while untrusted `BASE..HEAD` input remains bounded. Generated changes on side branches and in the PR must carry exact canonical release commits; a squash- or rebase-rewritten base transition is recoverable only when its complete entry tuple uniquely matches a canonical successor and advances monotonically. A non-monotonic rewrite fails closed and must be corrected with forward or commit-preserving history. Every merge's pair of full Git tree entries must exactly match a parent, and the no-change path runs only when the head entries exactly match the validated base. Hidden edits, synthetic merge resolutions, duplicate or copied tuples, and rollbacks fail.
