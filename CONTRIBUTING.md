...
- Do not hand-edit `CHANGELOG.md` or `.release-please-manifest.json`. Release Please regenerates both from conventional commits. The base-controlled `Generated files must not be hand-edited` check accepts only exact canonical output or an authenticated pending Release Please head, and rejects edit-then-restore histories, stale copies, and rollbacks. The legacy check still exempts the release bots during bootstrap, but that exemption does not bypass the base-controlled check.
