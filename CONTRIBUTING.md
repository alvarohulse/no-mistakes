...
- Do not hand-edit `CHANGELOG.md` or `.release-please-manifest.json`. During the provenance-guard bootstrap, the legacy pull-request check rejects any PR that touches either file unless its author is one of the two exempted release bots, while the base-controlled check inspects every head-side commit before accepting the no-change fast path, so a hand edit cannot be hidden by restoring the file later. Once the legacy check is retired, the base-controlled check will allow canonical syncs only when both files retain the exact full Git tree entries from one unambiguous, single-parent release-only commit reachable from `kunchenguid/no-mistakes` `main`; any number of other canonical commits may accompany it.
