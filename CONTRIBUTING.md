...
- Do not hand-edit `CHANGELOG.md` or `.release-please-manifest.json`. During the provenance-guard bootstrap, the legacy pull-request check still exempts the two release bots, but the base-controlled check independently validates canonical upstream before any fast path and fails closed if that provenance cannot be established. After the exact pinned mainline adoption, trusted history is audited without a lifetime ceiling and untrusted PR history is bounded. Ordinary PRs may incorporate generated changes only through exact canonical release commits; only an authenticated same-repository release-please PR may introduce one pending commit directly on the current base, changing only these two files. Squash- or rebase-rewritten trusted mainline transitions are recoverable only when their complete entry tuple uniquely matches a monotonic canonical successor. Restored hand edits, synthetic merge entries, duplicate canonical tuples, untrusted copies, and rollbacks fail.
