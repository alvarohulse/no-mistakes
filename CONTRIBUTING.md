...
- Do not hand-edit `CHANGELOG.md` or `.release-please-manifest.json`. During the provenance-guard bootstrap, the legacy pull-request check rejects any PR that touches either file unless its author is one of the two exempted release bots. The base-controlled check audits every `BASE..HEAD` commit before any fast path: every non-merge generated-entry change must be a canonical, single-parent, release-only commit with regular non-executable blobs, and every merge's pair of full Git tree entries must exactly match a parent without lowering the latest canonical provenance in its history. It takes the no-change path only when the head entries exactly match the validated base; otherwise the head must uniquely carry a release candidate from canonical `kunchenguid/no-mistakes` `main` that is not older than the canonical provenance carried by the base. Hidden edits, synthetic merge resolutions, copied blobs, and rollbacks fail.
