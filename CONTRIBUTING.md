...
- Do not hand-edit `CHANGELOG.md` or `.release-please-manifest.json`. During the provenance-guard bootstrap, the legacy pull-request check still exempts the two release bots, but the base-controlled check independently validates canonical upstream before any fast path and fails closed if that provenance cannot be established. Ordinary PRs may incorporate generated changes only through exact canonical release commits. Because the default `GITHUB_TOKEN` suppresses release-PR workflow events, the trusted release workflow requires `main` to remain at its triggering SHA, privately retains the exact two blobs produced through the pinned Release Please dependency graph, compares them with the same-repository `release-please--branches--main` head, runs the base-owned guard, and publishes success plus a namespaced provenance tag only for that SHA; a later force-push loses the status and must be attested independently. After the exact pinned mainline adoption, trusted first-parent and side history is audited without a lifetime ceiling, with squash- or rebase-rewritten tuples carried forward only when they match a durably attested release head and preserve monotonic logical provenance. Untrusted PR history remains bounded. Restored hand edits, stale merge results, synthetic merge entries, duplicate canonical tuples, untrusted copies, and rollbacks fail.
