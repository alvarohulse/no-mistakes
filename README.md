...
- **Agent-agnostic** - `claude`, `codex`, `rovodev`, `opencode`, `pi`, `copilot`, or `cursor`, plus explicit `acp:<target>` runners through `acpx`, with ordered fallbacks; every gate requires a runnable configured pipeline agent.
