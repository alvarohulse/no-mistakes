...
type Run struct {
	...
	IntentScore     *float64
	// PRNote is optional trusted author content rendered verbatim in the PR body.
	PRNote    *string
	CreatedAt int64
	UpdatedAt int64
}

const runColumns = `id, repo_id, branch, head_sha, base_sha, status, pr_url, error, awaiting_agent_since, COALESCE(parked_ms, 0), intent, intent_source, intent_session_id, intent_score, pr_note, created_at, updated_at`
