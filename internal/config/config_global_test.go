...
`
	...
	if cfg.Prompts.Refresh != "global refresh\n" {
		t.Errorf("prompts.refresh = %q", cfg.Prompts.Refresh)
	}
	if cfg.Prompts.Review != "global review\n" {
		t.Errorf("prompts.review = %q", cfg.Prompts.Review)
	}
	if cfg.Prompts.Build != "global build\n" {
		t.Errorf("prompts.build = %q", cfg.Prompts.Build)
	}
...
func TestDefaultConfigYAML_MatchesGoDefaults(t *testing.T) {
	...
	if raw.AutoFix.Refresh == nil || *raw.AutoFix.Refresh != defaults.Refresh {
		t.Errorf("YAML auto_fix.refresh = %v, Go default = %d", raw.AutoFix.Refresh, defaults.Refresh)
	}
}
...
func TestLoadGlobal_AutoFixDefaults(t *testing.T) {
	...
	if cfg.AutoFix.Lint != nil || cfg.AutoFix.Test != nil || cfg.AutoFix.Review != nil ||
		cfg.AutoFix.Document != nil || cfg.AutoFix.CI != nil || cfg.AutoFix.Refresh != nil {
