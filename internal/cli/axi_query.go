...
func newAxiLogsCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&step, "step", "", "step name: intent, rebase, review, test, document, retrospect, lint, push, pr, ci (required)")
...
func runAxiLogs(cmd *cobra.Command, step, runID string, full bool) error {
	...
	if step == "" {
		return emitError(cmd, 2, "--step is required",
			"Valid steps: intent, rebase, review, test, document, retrospect, lint, push, pr, ci")
	}
