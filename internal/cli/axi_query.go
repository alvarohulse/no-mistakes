...
func startRunHelp() string {
	return `Run no-mistakes axi run --intent "the user's goal" --yes to validate the current branch; optional --pr-note (or --pr-note-file <path> for longer content) injects author text into the PR`
}
