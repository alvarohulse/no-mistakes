...
func TestRefreshStep_NormalPushSyncsOriginBranch(t *testing.T) {
	...
	for _, operation := range operations {
		...
		if operation.Decision != db.RefreshDecisionFastForwarded || len(operation.CommandAttemptIDs) != 1 || operation.StartingHeadSHA != headSHA || operation.ResultingHeadSHA != originFeatureSHA {
