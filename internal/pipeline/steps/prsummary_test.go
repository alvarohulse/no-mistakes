...
func TestBuildPipelineSummary_EmitsStructuredStepAttestation(t *testing.T) {
	...
	steps := []*db.StepResult{
		...
		{ID: "refresh", StepName: types.StepRefresh, Status: types.StepStatusCompleted},
	...
	want := []struct {
		...
		{types.StepRefresh, types.StepStatusCompleted},
	...
	if attestationAt, telemetryAt := strings.Index(got, prefix), strings.Index(got, pipelineAgentTelemetryTableHeader); telemetryAt < attestationAt {
		t.Fatalf("attestation must precede optional telemetry so truncation retains it:\n%s", got)
	}
}
...
func TestBuildPipelineSummary_RendersStoredCommandEvidenceAndExplanation(t *testing.T) {
	...
	md, _ := BuildPipelineSummary(steps, map[string][]*db.StepRound{"build": {}}, testPipelineHeadSHA)
...
func TestBuildPipelineSummary_RendersStoredIntentEvidence(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			md, _ := BuildPipelineSummary(steps, map[string][]*db.StepRound{"intent": {}}, testPipelineHeadSHA)
...
func TestBuildTestingSummaryForPR_ReportsLocalOnlyMediaWithoutAttachmentFallback(t *testing.T) {
	t.Parallel()
	repoRoot := t.TempDir()
	...
	wantBytes := []byte("local screenshot bytes")
	if err := os.WriteFile(localPath, wantBytes, 0o644); err != nil {
		t.Fatal(err)
	}
	...
	md := BuildTestingSummaryForPR(steps, rounds, "git@github.com:example/widgets.git", "abc123", repoRoot, evidenceRoot)
	t.Logf("rendered PR testing markdown:\n%s", md)

	want := "- Evidence: Checkout screenshot (local-only; unavailable from the pull request)"
	if !strings.Contains(md, want) {
		t.Fatalf("expected local media availability to render without a remote fallback, got:\n%s", md)
	}
	if strings.Contains(md, localPath) {
		t.Fatalf("PR body exposed owner-local evidence path %q:\n%s", localPath, md)
	}
	for _, broken := range []string{"![Checkout screenshot]", "github.com/example/widgets/blob/abc123/", "github.com/user-attachments/", "raw.githubusercontent.com"} {
		if strings.Contains(md, broken) {
	...
	if err != nil {
		t.Fatalf("local evidence was not retained: %v", err)
	}
	if !bytes.Equal(gotBytes, wantBytes) {
		t.Fatalf("local evidence changed: got %q, want %q", gotBytes, wantBytes)
	}
}

func TestBuildTestingSummaryForPR_ReportsCaptionedLocalVisualArtifactWithoutPath(t *testing.T) {
	t.Parallel()
	repoRoot := t.TempDir()
	evidenceRoot := filepath.Join(t.TempDir(), "evidence", "run-123")
	localPath := filepath.Join(evidenceRoot, "checkout.png")
	...
	wantSource := "Source: Checkout screenshot (local-only; unavailable from the pull request)"
	if !strings.Contains(md, wantSource) {
		t.Fatalf("expected captioned local media availability, got:\n%s", md)
	}
	if strings.Contains(md, localPath) {
		t.Fatalf("PR body exposed owner-local evidence path %q:\n%s", localPath, md)
	}
...
// TestBuildTestingSummaryForPR_RejectsAnotherRunsEvidenceArtifact pins the run
// scoping of embedded evidence: the temp evidence root is shared by every run
// on the machine, so a path naming a sibling run's directory must neither be
// linked nor have its contents published in this run's PR body.
func TestBuildTestingSummaryForPR_RejectsAnotherRunsEvidenceArtifact(t *testing.T) {
	otherRunSecret := "OTHER RUN PRIVATE EVIDENCE"
	evidenceRoot := t.TempDir()
	thisRunDir := filepath.Join(evidenceRoot, "run-current")
	otherRunDir := filepath.Join(evidenceRoot, "run-other")
	if err := os.MkdirAll(otherRunDir, 0o755); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = os.RemoveAll(otherRunDir) })
	otherRunPath := filepath.Join(otherRunDir, "leak.txt")
	if err := os.WriteFile(otherRunPath, []byte(otherRunSecret), 0o644); err != nil {
		t.Fatal(err)
	}

	findings := fmt.Sprintf(`{"findings":[],"summary":"","testing_summary":"Evidence was collected.","artifacts":[{"label":"Neighbour run log","path":%q}]}`, otherRunPath)
	steps := []*db.StepResult{
		{ID: "s1", StepName: types.StepTest, Status: types.StepStatusCompleted, FindingsJSON: &findings},
	}
	rounds := map[string][]*db.StepRound{
	...
	md := BuildTestingSummaryForPR(steps, rounds, "git@github.com:example/widgets.git", "abc123", t.TempDir(), thisRunDir)

	for _, leaked := range []string{otherRunSecret, otherRunPath, "Neighbour run log"} {
		if strings.Contains(md, leaked) {
			t.Fatalf("another run's evidence %q leaked into the PR testing section:\n%s", leaked, md)
		}
	}
}
...
func TestBuildTestingSummaryForPR_EmbedsLocalTextEvidenceContent(t *testing.T) {
	...
	md := BuildTestingSummaryForPR(steps, rounds, "git@github.com:example/widgets.git", "abc123", t.TempDir(), evidenceRoot)
	t.Logf("rendered PR testing markdown:\n%s", md)
...
func TestBuildTestingSummaryForPR_LimitsTotalEmbeddedTextEvidence(t *testing.T) {
	...
	if !strings.Contains(md, "- Evidence: Third log (local-only; unavailable from the pull request)") || strings.Contains(md, thirdPath) {
...
func TestBuildTestingSummaryForPR_FallsBackForBinaryEvidence(t *testing.T) {
	...
	if !strings.Contains(md, "- Evidence: Binary capture (local-only; unavailable from the pull request)") || strings.Contains(md, localPath) {
		t.Fatalf("expected binary evidence to fall back to a local-only availability note, got:\n%s", md)
	}
	if strings.Contains(md, "```text") {
