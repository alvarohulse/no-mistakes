...
func TestTestStep_FixMode_UsesExplicitReplacementForPersistedPlannedCommand(t *testing.T) {
	t.Parallel()
	dir, baseSHA, headSHA := setupGitRepo(t)
	gitCmd(t, dir, "checkout", "--detach", headSHA)

	const invalidCommand = "false"
	const replacementCommand = "true"
	ag := &mockAgent{
		name: "test",
		runFn: func(_ context.Context, opts agent.RunOpts) (*agent.Result, error) {
			if opts.Purpose == "test-fix" {
				return &agent.Result{Output: json.RawMessage(`{"summary":"repair planned command","replacement_command":"true"}`)}, nil
			}
			return &agent.Result{Output: json.RawMessage(`{"findings":[],"summary":"evidence passed","tested":["true"],"testing_summary":"replacement passed"}`)}, nil
		},
	}
	...
	sctx.PlannedCommand = invalidCommand
	if err := sctx.DB.SetStepPlannedCommand(step.ID, invalidCommand); err != nil {
		t.Fatal(err)
	}
	sctx.Fixing = true
	...
	if outcome.NeedsApproval {
		t.Fatalf("replacement command should pass controller validation: %+v", outcome)
	}
	stored, err := sctx.DB.GetStepResult(step.ID)
	if err != nil {
		t.Fatal(err)
	}
	if stored.PlannedCommand == nil || *stored.PlannedCommand != replacementCommand {
		t.Fatalf("stored planned command = %v, want %q", stored.PlannedCommand, replacementCommand)
	}
	evidence, err := stored.Evidence()
	if err != nil {
		t.Fatal(err)
	}
	if len(evidence.Commands) != 1 || evidence.Commands[0].Command != replacementCommand || evidence.Commands[0].Outcome != db.CommandOutcomePassed {
		t.Fatalf("command evidence = %+v, want successful replacement command", evidence.Commands)
	}
}
...
func TestTestStep_FixMode_AgentWritesNewTests_RecordsInformationalFinding(t *testing.T) {
	t.Parallel()
	dir, baseSHA, headSHA := setupGitRepo(t)
	...
	if outcome.NeedsApproval {
		t.Error("expected no approval when the agent writes a new test file")
	}
	if callCount != 1 {
	...
	for _, item := range f.Items {
		if strings.Contains(item.Description, "component behavior.spec.tsx") {
			...
			if item.Severity != "info" {
				t.Errorf("expected new-test-file finding severity info, got %q", item.Severity)
			}
...
func TestTestStep_FixModeTestChangesSurviveEvidenceAgent(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if outcome.NeedsApproval {
				t.Fatal("expected test-file changes not to require approval after evidence agent")
			}
			var findings Findings
			if err := json.Unmarshal([]byte(outcome.Findings), &findings); err != nil {
				t.Fatal(err)
			}
			if !test.wantInfo && len(findings.Items) != 0 {
				t.Fatalf("modified test-file findings = %+v, want none", findings.Items)
			}
			if test.wantInfo && (len(findings.Items) != 1 || findings.Items[0].File != test.file || findings.Items[0].Severity != "info" || findings.Items[0].Action != types.ActionNoOp) {
				t.Fatalf("new test-file findings = %+v, want one informational finding", findings.Items)
			}
		})
	}
}

func TestTestStep_ConfiguredTestFailureNeedsApproval(t *testing.T) {
	t.Parallel()
	dir, baseSHA, headSHA := setupGitRepo(t)
	...
	if !outcome.NeedsApproval || !outcome.AutoFixable || outcome.ExitCode != 1 {
		t.Fatalf("test failure outcome = %+v, want auto-fixable approval with exit code 1", outcome)
	}
	var findings Findings
	if err := json.Unmarshal([]byte(outcome.Findings), &findings); err != nil {
		t.Fatal(err)
...
func TestTestStep_EvidenceAgentExistingTestFileMutationsDoNotGate(t *testing.T) {
	tests := []struct {
		name     string
		oldPath  string
		newPath  string
	...
	for _, tt := range tests {
		for _, staged := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				if outcome.NeedsApproval {
					t.Fatal("expected no approval when the agent mutates an existing test file")
				}
				...
				if len(findings.Items) != 0 {
					t.Fatalf("expected no test-file findings, got %+v", findings.Items)
				}
			})
		}
	}
