...
func replayOne(ctx context.Context, store *Store, c Case, session Session, candidate Candidate, repeat int) (evaluation Evaluation) {
	...
	if observed.result != nil {
		...
		applyEvaluationUsage(&evaluation, string(candidate.Agent), observed.result)
	}
	if err != nil {
...
func applyEvaluationUsage(evaluation *Evaluation, agentName string, result *agent.Result) {
	if evaluation == nil || result == nil {
		return
	}
	usage := result.Usage
	...
	if !usage.InputIsReported() || !usage.OutputIsReported() {
		return
	}
	input := usage.InputTokens
	var cacheRead *int
	if usage.CacheReadIsReported() {
	...
	_, freshInput := agent.CanonicalInputMeters(agentName, &input, cacheRead, cacheCreation)
	if freshInput == nil {
		return
	}

	evaluation.TokensReported = true
	evaluation.InputTokens = int64(input)
	evaluation.OutputTokens = int64(usage.OutputTokens)
	if cacheRead != nil {
		evaluation.CacheReadTokens = int64(*cacheRead)
	}
	evaluation.FreshInputTokens = int64(*freshInput)
}
