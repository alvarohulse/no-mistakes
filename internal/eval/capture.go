...
func sourceInvocationsFor(invocations []db.AgentInvocation) []sourceInvocation {
	...
	for _, inv := range invocations {
		out = append(out, sourceInvocation{
			...
			CacheCreationTokens: inv.CacheCreationTokens, FreshInputTokens: inv.FreshInputTokens,
			ReasoningTokens: inv.ReasoningTokens, DeltaInputTokens: inv.DeltaInputTokens,
			DeltaOutputTokens: inv.DeltaOutputTokens, DeltaCacheReadTokens: inv.DeltaCacheReadTokens,
			DeltaCacheCreationTokens: inv.DeltaCacheCreationTokens, ReportedCostUSD: inv.ReportedCostUSD,
			ModelRoundtrips: inv.ModelRoundtrips, ToolCalls: inv.ToolCalls, ToolWaitCalls: inv.ToolWaitCalls,
			ToolTestLintCalls: inv.ToolTestLintCalls, ToolEditCalls: inv.ToolEditCalls,
			ToolReadCalls: inv.ToolReadCalls, ToolGitCalls: inv.ToolGitCalls, ToolOtherCalls: inv.ToolOtherCalls,
			WorkloadFiles: inv.WorkloadFiles, WorkloadLines: inv.WorkloadLines, FindingCount: inv.FindingCount,
			ExitStatus: inv.ExitStatus, FailureCategory: inv.FailureCategory,
		})
	}
	return out
}
...
func baselineForRound(invocations []db.AgentInvocation, round int) BaselineMetrics {
	...
	for _, inv := range invocations {
		...
		if inv.DeltaInputTokens == nil || inv.DeltaOutputTokens == nil {
			complete = false
			continue
		}
		_, freshInput := agent.CanonicalInputMeters(inv.Agent, inv.DeltaInputTokens, inv.DeltaCacheReadTokens, inv.DeltaCacheCreationTokens)
		if freshInput == nil {
			complete = false
			continue
		}
		...
		baseline.FreshInputTokens += int64(*freshInput)
	}
