...
Optional ` + "`--pr-note`" + ` (or ` + "`--pr-note-file <path>`" + ` for longer content; mutually exclusive) injects your own text into the pull request the ` + "`pr`" + ` step opens. Each input is limited to 47 KiB. After surrounding whitespace is trimmed, the note is reproduced verbatim in a ` + "`## Notes`" + ` section of the PR body, placed after ` + "`## Intent`" + ` and before ` + "`## What Changed`" + `, and is also fed to the PR summary prompt as trusted author guidance. If the note already starts with a ` + "`## Notes`" + ` heading (case-insensitive), no second heading is added; other level-two headings inside it remain part of the atomic note. Unlike inferred intent, the note receives no untrusted-data framing, adversarial stripping, or secret redaction, so do not include secrets. It is run-scoped like ` + "`--intent`" + `: the note persists on the run and ` + "`axi run`" + ` reuses it when reattaching to or re-triggering the same head, but ` + "`no-mistakes rerun`" + ` and the TUI rerun start a fresh run without the note. When the PR body must be truncated, generated and pipeline sections are clamped before the author note; the note itself is clamped only when it alone exceeds the host limit.
