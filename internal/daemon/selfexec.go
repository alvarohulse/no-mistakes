...
func reinstallManagedServiceIfChanged(p *paths.Paths) (bool, error) {
	...
	switch runtimeGOOS {
	case "darwin":
		wanted = renderLaunchAgentWithForwardedEnv(renderedExecutable, p, home, proxyEnv)
	case "linux":
		wanted = renderSystemdUnitWithForwardedEnv(renderedExecutable, p, home, proxyEnv)
	}
	switch {
...
func stopCurrentDaemonBeforeManagedRestart(p *paths.Paths) error {
	...
	managed, managedErr := stopManagedService(p)
	var detachedErr error
	// A managed service definition can coexist with a detached daemon. Stop
	// whichever daemon still answers before restarting the managed service.
	if alive, _ := daemonHealthCheck(p); alive {
		detachedErr = stopDetachedDaemon(p)
	}
	if waitErr := waitForDaemonStop(p, instance); waitErr != nil {
		switch {
		case managed && managedErr != nil && detachedErr != nil:
			return fmt.Errorf("stop managed daemon before restart: %w; detached shutdown: %v; wait for exit: %v", managedErr, detachedErr, waitErr)
		case managed && managedErr != nil:
			return fmt.Errorf("stop managed daemon before restart: %w; wait for exit: %v", managedErr, waitErr)
		case detachedErr != nil:
			return fmt.Errorf("detached shutdown before managed restart: %w; wait for exit: %v", detachedErr, waitErr)
		default:
			return fmt.Errorf("wait for daemon exit before managed restart: %w", waitErr)
		}
	}
...
func waitForDaemonStop(p *paths.Paths, captured ...daemonInstance) error {
