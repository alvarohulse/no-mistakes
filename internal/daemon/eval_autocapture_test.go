...
func TestRecoveredRunCompletionAutoCapturesEligibleReview(t *testing.T) {
	ctx := context.Background()
	p, database, runID := setupFinishedReviewRun(t, ctx)
	...
	m := NewRunManager(database, p, func() []pipeline.Step { return nil })

	m.resumeRecoveredRun(recoveredRunPlan{
		run: run, repo: repo, workDir: t.TempDir(), gateDir: t.TempDir(),
		cfg: evalConfig(true, true), agent: recoveredRunTestAgent{},
	})
	m.wg.Wait()

	if got := capturedCaseCount(t, p); got != 1 {
		t.Fatalf("recovered completion collected %d cases, want 1", got)
	}
}
