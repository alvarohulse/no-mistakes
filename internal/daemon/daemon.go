...
func registerHandlers(srv *ipc.Server, mgr *RunManager, d *db.DB, shutdown func()) {
	...
	srv.Handle(ipc.MethodRerun, func(ctx context.Context, params json.RawMessage) (interface{}, error) {
		...
		runID, err := mgr.HandleRerunWithOverrides(ctx, p.RepoID, p.Branch, p.PreviousRunID, p.SkipSteps, p.Intent, p.PRNote, p.Metadata, p.EffectiveConfigPublish, p.RefreshStrategy, p.StackedOn)
		if err != nil {
			return nil, err
		}
...
func runToInfo(d *db.DB, r *db.Run, steps []*db.StepResult) *ipc.RunInfo {
	info := &ipc.RunInfo{
		...
		BaseSHA:            r.BaseSHA,
		RefreshStrategy:    r.RefreshStrategy,
		StackedOn:          r.StackedOn,
		Status:             r.Status,
		PRURL:              r.PRURL,
		Error:              r.Error,
