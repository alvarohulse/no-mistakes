	...
	for i, step := range steps {
		...
		case types.StepStatusSkipped:
			if step.SkipSource != nil {
				line += " " + dimStyle.Render("- "+*step.SkipSource)
			}
		case types.StepStatusFailed:
			if step.Error != nil {
				errText := "- " + *step.Error
...
func renderActionBar(steps []ipc.StepResultInfo, strategy types.RefreshStrategy, showSelectionActions bool, allowFix bool, showDiff bool, selectedCount int, totalCount int, confirmAbort bool, hasDiff bool, approvalReady bool, retryAvailable bool) string {
