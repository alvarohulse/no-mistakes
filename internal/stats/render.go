...
func renderText(report *Report, forceDetails bool) string {
	...
	for _, agent := range report.Agents {
		...
		fmt.Fprintf(&out, "agent %s/%s step=%s round=%d purpose=%s harness=%s usage_coverage=%s model=%s effort=%s provider=%s status=%s session=%s duration_ms=%d delta_input_tokens=%s raw_input_tokens=%s cache_write_tokens=%s reported_cost_usd=%s subprocess_wait_ms=%s roundtrips=%s tool_calls=%s test_lint_calls=%s workload=%s/%s\n",
			agent.RunID, invocation.ID, invocation.Step, invocation.Round, invocation.Purpose, invocation.Agent, invocation.UsageCoverage, textString(invocation.Model), textString(invocation.Effort), textString(invocation.Provider), invocation.ExitStatus, invocation.SessionMode, invocation.DurationMS,
