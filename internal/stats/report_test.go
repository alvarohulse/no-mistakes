...
func TestBuildReportCombinesAllDirectFiltersAndSections(t *testing.T) {
	...
	if len(report.Runs.Items) != 1 || len(report.Steps) != 1 || len(report.Agents) != 1 || len(report.Repairs) != 1 || len(report.Metrics.Items) != 1 {
		t.Fatalf("report sections = runs %d steps %d agents %d repairs %d metrics %d", len(report.Runs.Items), len(report.Steps), len(report.Agents), len(report.Repairs), len(report.Metrics.Items))
	}
	if report.Steps[0].Step.Name != types.StepReview || report.Agents[0].Invocation.Agent != "codex" {
