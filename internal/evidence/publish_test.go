...
func TestPublish_DoesNotCollideWithAnotherPublicationFetchRef(t *testing.T) {
	...
	competingRef := "refs/no-mistakes/evidence-fetch/concurrent"
	...
	if refs := runGit(t, work, "for-each-ref", "--format=%(refname)", "refs/no-mistakes/evidence-fetch"); refs != competingRef {
