...
// TestRunSelectionFollowsReturnedCustodyIntoNewerPush reproduces stacked
// terminal custody: an older failed run left an unpublished head, a newer run
// rebased that exact head and returned custody, and a final run rebased and
// pushed local follow-up work. Neither rebase preserves Git ancestry, so the
// durable submitted-head and custody chain must keep the older run from
// reclaiming authority.
func TestRunSelectionFollowsReturnedCustodyIntoNewerPush(t *testing.T) {
	t.Parallel()

	f := newRebasedRecoverFixture(t, types.RunFailed)
	older := f.run

	// Advance the base again, then model the newer run rebasing the older
	// preserved head. This deliberately makes the two run heads diverge by
	// ancestry even though the newer run's submitted head records the exact
	// custody lineage.
	mustRun(t, f.local, "checkout", "main")
	mustWrite(t, filepath.Join(f.local, "upstream-2.txt"), "second upstream advance\n")
	mustRun(t, f.local, "add", "upstream-2.txt")
	mustRun(t, f.local, "commit", "-m", "second upstream advance")
	newBase := mustRun(t, f.local, "rev-parse", "HEAD")
	mustRun(t, f.local, "push", f.gate, "refs/heads/main:refs/heads/main")
	mustRun(t, f.local, "checkout", "feature/recover")

	newerWorktree := filepath.Join(t.TempDir(), "newer-pipeline")
	mustRun(t, filepath.Dir(newerWorktree), "-c", "core.autocrlf=false", "clone", f.gate, newerWorktree)
	configureIdentity(t, newerWorktree)
	mustRun(t, newerWorktree, "checkout", "feature/recover")
	mustRun(t, newerWorktree, "rebase", "origin/main")
	newerHead := mustRun(t, newerWorktree, "rev-parse", "HEAD")
	mustRun(t, newerWorktree, "push", "--force", "origin", "HEAD:refs/heads/feature/recover")
	if isAncestor(f.ctx, f.gate, older.HeadSHA, newerHead) {
		t.Fatal("fixture did not reproduce rebased run-head divergence")
	}

	time.Sleep(1100 * time.Millisecond)
	unrelated, err := f.db.InsertRun(f.repo.ID, "feature/recover", f.submitted, newBase)
	if err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunHeadSHA(unrelated.ID, newerHead); err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunStatusWithVerifiedHead(unrelated.ID, types.RunFailed, newerHead); err != nil {
		t.Fatal(err)
	}
	if err := f.db.SetRunCustodyReturned(unrelated.ID); err != nil {
		t.Fatal(err)
	}
	if state := f.service.InspectCached(f.ctx); state.Pipeline.RunID != older.ID {
		t.Fatalf("unrelated returned custody superseded older run: pipeline %#v", state.Pipeline)
	}
	mustRun(t, newerWorktree, "push", "--force", f.remote, "HEAD:refs/heads/feature/recover")
	time.Sleep(1100 * time.Millisecond)
	unrelatedPushed, err := f.db.InsertRun(f.repo.ID, "feature/recover", f.submitted, newBase)
	if err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunHeadSHA(unrelatedPushed.ID, newerHead); err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunPushBinding(unrelatedPushed.ID, db.PushBinding{
		HeadSHA: newerHead, TargetKind: "upstream", TargetFingerprint: TargetFingerprint(f.remote), Ref: "refs/heads/feature/recover",
	}); err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunStatusWithVerifiedHead(unrelatedPushed.ID, types.RunCompleted, newerHead); err != nil {
		t.Fatal(err)
	}
	if state := f.service.InspectCached(f.ctx); state.Pipeline.RunID != older.ID {
		t.Fatalf("unrelated pushed binding superseded older run: pipeline %#v", state.Pipeline)
	}

	time.Sleep(1100 * time.Millisecond)
	newer, err := f.db.InsertRun(f.repo.ID, "feature/recover", older.HeadSHA, newBase)
	if err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunHeadSHA(newer.ID, newerHead); err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunStatusWithVerifiedHead(newer.ID, types.RunFailed, newerHead); err != nil {
		t.Fatal(err)
	}
	...
	// The operator accepted the newer gate lineage and then added follow-up
	// work locally. Recovery must remain an idempotent custody acknowledgement;
	// it must not move either the local branch or the gate backward.
	mustRun(t, f.local, "fetch", "--no-tags", f.gate, "+refs/heads/feature/recover:refs/no-mistakes/test/newer")
	mustRun(t, f.local, "reset", "--hard", newerHead)
	mustWrite(t, filepath.Join(f.local, "follow-up.txt"), "operator follow-up\n")
	...
	state := f.service.InspectCached(f.ctx)
	if state.Pipeline.RunID != newer.ID || state.State != StateCustodyReturned || state.Safety != "custody_returned" {
		t.Fatalf("selection with newer returned custody = %s/%s pipeline %#v, want newer run %s", state.State, state.Safety, state.Pipeline, newer.ID)
	}
	recovered := f.service.Recover(f.ctx, true)
	if !recovered.Recovered || recovered.Changed {
		t.Fatalf("idempotent recovery under newer returned custody = %#v", recovered)
	}
	if got := mustRun(t, f.local, "rev-parse", "HEAD"); got != localHead {
		t.Fatalf("idempotent recovery moved local head to %s, want %s", got, localHead)
	}
	if got := mustRun(t, f.gate, "rev-parse", "refs/heads/feature/recover"); got != newerHead {
		t.Fatalf("idempotent recovery moved gate head to %s, want %s", got, newerHead)
	}
	oldAfter, err := f.db.GetRun(older.ID)
	if err != nil {
		t.Fatal(err)
	}
	if oldAfter.CustodyReturnedAt != nil {
		t.Fatal("idempotent recovery stamped custody on the superseded older run")
	}

	// A later run submits the operator's follow-up head, rebases it onto another
	// base advance, and publishes the result. The exact push binding plus the
	// returned-custody chain must now win selection without moving either ref.
	mustRun(t, f.local, "checkout", "main")
	mustWrite(t, filepath.Join(f.local, "upstream-3.txt"), "third upstream advance\n")
	mustRun(t, f.local, "add", "upstream-3.txt")
	mustRun(t, f.local, "commit", "-m", "third upstream advance")
	pushBase := mustRun(t, f.local, "rev-parse", "HEAD")
	mustRun(t, f.local, "push", f.gate, "refs/heads/main:refs/heads/main")
	mustRun(t, f.local, "checkout", "feature/recover")
	mustRun(t, f.local, "push", f.gate, "refs/heads/feature/recover:refs/heads/feature/recover")

	pushedWorktree := filepath.Join(t.TempDir(), "pushed-pipeline")
	mustRun(t, filepath.Dir(pushedWorktree), "-c", "core.autocrlf=false", "clone", f.gate, pushedWorktree)
	configureIdentity(t, pushedWorktree)
	mustRun(t, pushedWorktree, "checkout", "feature/recover")
	mustRun(t, pushedWorktree, "rebase", "origin/main")
	pushedHead := mustRun(t, pushedWorktree, "rev-parse", "HEAD")
	mustRun(t, pushedWorktree, "push", "--force", "origin", "HEAD:refs/heads/feature/recover")
	mustRun(t, pushedWorktree, "push", "--force", f.remote, "HEAD:refs/heads/feature/recover")
	if isAncestor(f.ctx, f.gate, localHead, pushedHead) {
		t.Fatal("fixture did not reproduce pushed rebase divergence")
	}

	time.Sleep(1100 * time.Millisecond)
	pushed, err := f.db.InsertRun(f.repo.ID, "feature/recover", localHead, pushBase)
	if err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunHeadSHA(pushed.ID, pushedHead); err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunPushBinding(pushed.ID, db.PushBinding{
		HeadSHA: pushedHead, TargetKind: "upstream", TargetFingerprint: TargetFingerprint(f.remote), Ref: "refs/heads/feature/recover",
	}); err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunStatusWithVerifiedHead(pushed.ID, types.RunCompleted, pushedHead); err != nil {
		t.Fatal(err)
	}

	state = f.service.InspectCached(f.ctx)
	if state.Pipeline.RunID != pushed.ID {
		t.Fatalf("selection after newer exact push = %#v, want pushed run %s", state.Pipeline, pushed.ID)
	}
	if state.NextAction != nil && state.NextAction.Code == "recover_custody" {
		t.Fatalf("newer exact push still offered older recovery: %#v", state)
	}
	if got := mustRun(t, f.local, "rev-parse", "HEAD"); got != localHead {
		t.Fatalf("selection moved local head to %s, want %s", got, localHead)
	}
	if got := mustRun(t, f.gate, "rev-parse", "refs/heads/feature/recover"); got != pushedHead {
		t.Fatalf("selection moved gate head to %s, want %s", got, pushedHead)
	}
	if got := mustRun(t, f.gate, "show", pushedHead+":follow-up.txt"); got != "operator follow-up" {
		t.Fatalf("pushed lineage lost operator follow-up: %q", got)
	}

	// The operator synchronizes the first pushed rebase, adds another bounded
	// follow-up, and a final run rebases and pushes again before its monitor is
	// cancelled. Selection must traverse both exact pushed transformations.
	mustRun(t, f.local, "fetch", "--no-tags", f.gate, "+refs/heads/feature/recover:refs/no-mistakes/test/pushed")
	mustRun(t, f.local, "reset", "--hard", pushedHead)
	mustWrite(t, filepath.Join(f.local, "follow-up-2.txt"), "second operator follow-up\n")
	mustRun(t, f.local, "add", "follow-up-2.txt")
	mustRun(t, f.local, "commit", "-m", "second operator follow-up")
	finalSubmitted := mustRun(t, f.local, "rev-parse", "HEAD")
	mustRun(t, f.local, "checkout", "main")
	mustWrite(t, filepath.Join(f.local, "upstream-4.txt"), "fourth upstream advance\n")
	mustRun(t, f.local, "add", "upstream-4.txt")
	mustRun(t, f.local, "commit", "-m", "fourth upstream advance")
	finalBase := mustRun(t, f.local, "rev-parse", "HEAD")
	mustRun(t, f.local, "push", f.gate, "refs/heads/main:refs/heads/main")
	mustRun(t, f.local, "checkout", "feature/recover")
	mustRun(t, f.local, "push", "--force", f.gate, "refs/heads/feature/recover:refs/heads/feature/recover")

	finalWorktree := filepath.Join(t.TempDir(), "final-pipeline")
	mustRun(t, filepath.Dir(finalWorktree), "-c", "core.autocrlf=false", "clone", f.gate, finalWorktree)
	configureIdentity(t, finalWorktree)
	mustRun(t, finalWorktree, "checkout", "feature/recover")
	mustRun(t, finalWorktree, "rebase", "origin/main")
	finalPushedHead := mustRun(t, finalWorktree, "rev-parse", "HEAD")
	mustRun(t, finalWorktree, "push", "--force", "origin", "HEAD:refs/heads/feature/recover")
	mustRun(t, finalWorktree, "push", "--force", f.remote, "HEAD:refs/heads/feature/recover")
	if isAncestor(f.ctx, f.gate, finalSubmitted, finalPushedHead) {
		t.Fatal("fixture did not reproduce final pushed rebase divergence")
	}

	time.Sleep(1100 * time.Millisecond)
	finalPushed, err := f.db.InsertRun(f.repo.ID, "feature/recover", finalSubmitted, finalBase)
	if err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunHeadSHA(finalPushed.ID, finalPushedHead); err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunPushBinding(finalPushed.ID, db.PushBinding{
		HeadSHA: finalPushedHead, TargetKind: "upstream", TargetFingerprint: TargetFingerprint(f.remote), Ref: "refs/heads/feature/recover",
	}); err != nil {
		t.Fatal(err)
	}
	if err := f.db.UpdateRunStatusWithVerifiedHead(finalPushed.ID, types.RunCancelled, finalPushedHead); err != nil {
		t.Fatal(err)
	}

	state = f.service.InspectCached(f.ctx)
	if state.Pipeline.RunID != finalPushed.ID {
		t.Fatalf("selection after multi-hop exact push = %#v, want final run %s", state.Pipeline, finalPushed.ID)
	}
	if state.NextAction != nil && state.NextAction.Code == "recover_custody" {
		t.Fatalf("final exact push still offered older recovery: %#v", state)
	}
	if got := mustRun(t, f.local, "rev-parse", "HEAD"); got != finalSubmitted {
		t.Fatalf("multi-hop selection moved local head to %s, want %s", got, finalSubmitted)
	}
	if got := mustRun(t, f.gate, "rev-parse", "refs/heads/feature/recover"); got != finalPushedHead {
		t.Fatalf("multi-hop selection moved gate head to %s, want %s", got, finalPushedHead)
	}
	for path, want := range map[string]string{
		"follow-up.txt":   "operator follow-up",
		"follow-up-2.txt": "second operator follow-up",
	} {
		if got := mustRun(t, f.gate, "show", finalPushedHead+":"+path); got != want {
			t.Fatalf("final pushed lineage lost %s: %q", path, got)
		}
	}
}
